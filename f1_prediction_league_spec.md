...
- Generation is lazy, not a background job: it runs the moment anyone next touches that locked session — a user loading their event overview or that session's prediction page, or an admin viewing the results hub or entering/editing its results — so no separate scheduler is needed. This means a user sees their own auto-filled bet as soon as they open the locked session, without waiting for an admin to act first.
